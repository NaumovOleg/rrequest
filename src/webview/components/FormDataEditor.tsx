...
export function FormDataEditor() {
  ...
  const items: FormDataItem[] =
    active.body.mode === "formdata" ? active.body.items : [];
  const setItems = (next: FormDataItem[]) =>
    update({ body: { mode: "formdata", items: next } });
  const patch = (i: number, next: FormDataItem) =>
    setItems(items.map((r, j) => (j === i ? next : r)));
          ...
          <tr key={i}>
            <td>
              <input
                ...
                onChange={(e) =>
                  i < items.length &&
                  patch(i, { ...r, enabled: e.target.checked })
                }
              />
            </td>
            <td>
            ...
            <td>
              {r.kind === "text" ? (
                <input
                  className="rm-input"
                  aria-label={`form value ${i}`}
                  placeholder="value"
                  value={r.value}
                  onChange={(e) =>
                    i < items.length &&
                    patch(i, { ...r, value: e.target.value })
                  }
                />
              ) : (
