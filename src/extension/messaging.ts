...
export type RouterDeps = {
  ...
  syncControl?: { signIn(): Promise<void>; signOut(accountId?: string): Promise<void>; enable(workspaceId: string, accountId?: string): Promise<void>; syncNow(workspaceId: string): Promise<void>; syncAccount(accountId: string): Promise<void>; setPolling(workspaceId: string, enabled: boolean): Promise<void>; setSyncMode(workspaceId: string, mode: 'full' | 'pull' | 'push' | 'stop'): Promise<void> }
...
export function createRouter(deps: RouterDeps) {
  ...
  return async function route(msg: WebviewMessage): Promise<HostMessage | undefined> {
    ...
    switch (msg.type) {
      ...
      case 'setSyncMode': await deps.syncControl?.setSyncMode(msg.workspaceId, msg.mode); return undefined
      default:
        return undefined
    }
  }
